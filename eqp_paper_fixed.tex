...
\begin{algorithm}
	...
	\Input{$network\leftarrow$ a network with the topology described in section \ref{sec:basic_topology}.}
	\Input{$p\leftarrow$ the probability with which to replace a given connection.}
	\BlankLine
	\For{$\text{layer}$ \upshape{\textbf{in}} $\textit{network}$}{
		\For{$\text{pair of neurons}$ \upshape{\textbf{in}} $\textit{layer}$}{
			\textbf{connect} \textit{pair of neurons}\;
		}
	}
	\BlankLine
	$N\leftarrow\lfloor\log(1-p)/\log(\frac{N-1}{N})\rfloor$\;
	\For{$i\leftarrow 1$ \KwTo $N$}{
		\textit{existing pair} $:=$ randomly-chosen pair of connected neurons\;
		\textit{new pair} $:=$ randomly-chosen pair of unconnected neurons\;
		\textbf{remove} connection between neurons in \textit{existing pair}\;
		\textbf{add} connection between neurons in \textit{new pair}\;
	}
