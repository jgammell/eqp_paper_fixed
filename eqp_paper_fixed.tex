...
{
	$\text{Randomly select edge }e\in N$;\\
	$\text{Add edge between random unconnected pair of neurons}$;\\
...
\end{equation}
(approximating $N\approx N-1$ and $N-l+1\approx N-l$), so the ratio of these corrections will be
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
(approximating $\frac{(N-l)\delta +2s_0}{N\delta+2s_0}\approx 1$). We have observed that
\begin{equation}
